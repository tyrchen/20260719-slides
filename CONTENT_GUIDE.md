...
- `content.js`：76 页中文内容与页面顺序，日常改稿主要编辑这里。
- `visuals.js`：四张主题 SVG 视觉，可修改图形、标签和节点。
- `charts.js`：D3 动态图表的数据、布局和进入动画。
- `vendor/d3.v7.min.js`：本地 D3 7.9.0，保证断网可演示。
...
所有幻灯片标题都会自动移除手工换行，并根据可用宽度缩小到单行。需要动态图表时，在
内容中加入 `data-chart="图表名"`，并在 `charts.js` 中维护相应的 D3 渲染函数。
